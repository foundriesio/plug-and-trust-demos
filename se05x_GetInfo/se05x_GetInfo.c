...
static sss_status_t JCOP4_GetDataIdentify(void *conn_ctx)
{
    ...
    * (ReUsing same dummy buffers) */
    rxStatus = (smStatus_t)GP_Select(conn_ctx, (uint8_t *)&identifyRsp /* dummy */, 0, (uint8_t *)&identifyRsp, &dummyResponse16);
    if (rxStatus != SM_OK) {
        LOG_E("Could not select ISD.");
        goto cleanup;
    }

    rxStatus = (smStatus_t)smCom_TransceiveRaw(conn_ctx, (uint8_t *)cmd, sizeof(cmd), (uint8_t *)&identifyRsp, &prspLen);
...
static sss_status_t JCOP4_GetCPLCData(void *conn_ctx)
{
    ...
    rxStatus = (smStatus_t)smCom_TransceiveRaw(conn_ctx, (uint8_t *)cmd, sizeof(cmd), (uint8_t *)&cplc_data, &cplc_data_len);
